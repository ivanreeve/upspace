...
describe('provider webhook api', () => {
  ...
  it('syncs provider balance for paid invoices even when partner metadata is missing', async () => {
    process.env.XENDIT_WEBHOOK_VERIFICATION_TOKEN = 'test-callback-token';
    ...
    const walletTransactionCreate = vi.fn().mockResolvedValue({ id: 'wallet-tx-1', });
    ...
    vi.spyOn(prismaModule, 'prisma', 'get').mockReturnValue({
      ...
      wallet_transaction: { create: walletTransactionCreate, },
      partner_provider_account: {
        findUnique: vi.fn().mockResolvedValue(null),
        findFirst: vi.fn().mockResolvedValue({
          id: 'provider-record-fallback',
          partner_user_id: 42n,
          provider_account_id: 'acct-remote-fallback',
        }),
      },
    } as unknown as typeof prismaModule.prisma);
    ...
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({ received: true, });
    expect(getPartnerBalance).toHaveBeenCalledWith('acct-remote-fallback');
    expect(walletTransactionCreate).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ metadata: expect.objectContaining({ partner_provider_account_id: 'provider-record-fallback', }), }), })
    );
    expect(walletSnapshotsModule.recordPartnerWalletSnapshot).toHaveBeenCalledWith({
      partnerUserId: 42n,
      partnerProviderAccountId: 'provider-record-fallback',
      availableBalanceMinor: 150000n,
      currency: 'PHP',
      fetchedAt: new Date('2026-03-13T11:05:00.000Z'),
    });
