...
} from "@/hooks/useSpaceFormPersistence";
...
export function SpacesInventoryForm() {
  ...
  const [unpublishDialogSpaceId, setUnpublishDialogSpaceId] = useState<
    string | null
  >(null);
  const [unpublishReason, setUnpublishReason] = useState("");
  const { data: spaces, isLoading, isError, error } = usePartnerSpacesQuery();
  const requestUnpublishMutation = useRequestUnpublishSpaceMutation(
    unpublishDialogSpaceId ?? "",
  );
  const { summary: draftSummary, refresh: refreshDraftSummary } =
    useSpaceDraftSummary();
  ...
  const handleOpenUnpublishDialog = useCallback((spaceId: string) => {
    setUnpublishDialogSpaceId(spaceId);
    setUnpublishReason("");
  }, []);

  const handleCloseUnpublishDialog = useCallback(() => {
    if (requestUnpublishMutation.isPending) return;
    setUnpublishDialogSpaceId(null);
    setUnpublishReason("");
  }, [requestUnpublishMutation.isPending]);

  const handleSubmitUnpublishRequest = useCallback(async () => {
    if (!unpublishDialogSpaceId) return;
    try {
      await requestUnpublishMutation.mutateAsync({
        reason: unpublishReason.trim() || undefined,
      });
      toast.success("Unpublish request sent for review.");
      setUnpublishDialogSpaceId(null);
      setUnpublishReason("");
    } catch (err) {
      const message =
  ...
  }, [requestUnpublishMutation, unpublishDialogSpaceId, unpublishReason]);
                ...
                >
                  ...
                  <TableCell className="text-right space-x-2">
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => handleOpenUnpublishDialog(row.id)}
                      disabled={
                        row.status === "Unpublished" || row.pendingUnpublish
                      }
                    >
  ...
  return (
    ...
    >
      ...
      <Dialog
        open={Boolean(unpublishDialogSpaceId)}
        onOpenChange={(open) =>
          !open ? handleCloseUnpublishDialog() : undefined
        }
      >
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Tell us why you want to hide this listing. An admin will review
              and approve before it is removed from the marketplace.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2 mt-4">
            ...
            <Textarea
              ...
              placeholder="Share context for the admin reviewer"
            />
            <p className="text-xs text-muted-foreground">
          ...
          <DialogFooter className="gap-2 sm:gap-3">
            <Button
              type="button"
              variant="outline"
              onClick={handleCloseUnpublishDialog}
            ...
            <Button
              type="button"
              onClick={handleSubmitUnpublishRequest}
              disabled={requestUnpublishMutation.isPending}
            >
