...
const updateSchema = z.object({
  ...
  message: 'At least one of name or capacity must be provided',
});
...
export async function PUT(req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const { count } = await prisma.area.updateMany({
      where: { area_id: areaId, space_id: spaceId },
      data,
    });
    ...
    const updated = await prisma.area.findUnique({
      ...
      select: { area_id: true, space_id: true, name: true, capacity: true },
    });
